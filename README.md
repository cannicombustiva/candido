...
encoding — lives in the `JobTrackerCore` package, so `swift test` reaches it in
...
`JobTracker.xcodeproj` is **generated by `xcodegen` from `project.yml` and is not
...
The Debug build uses its own bundle identifier, `com.candido.JobTracker.dev`.
A sandboxed app's container is keyed by that identifier, so a Debug build gets
its own store and cannot open — or migrate — the applications a Release build is
tracking.
